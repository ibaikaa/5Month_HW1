...
final class NetworkLayer {
    ...
    func fetchProducts() async throws -> MainProductModel {
        guard let url = baseURL else {
            print("Error occured while trying to unwrap optional baseURL in fetchProductsData() method")
            return MainProductModel(products: [] )
        }
        let (data, _) = try await URLSession.shared.data(from: url)
        return try decoder(data: data)
    }
    
    // Find method with GET request changed to async
    func findProductsData(
        text: String
    ) async throws -> MainProductModel {
        guard let url = baseURL else {
            print("Error occured while trying to unwrap optional baseURL in findProductsData() NetworkLayer method")
            return MainProductModel(products: [])
        }
